...
def read_bead_params():
    ...
    bead_params["rvdw_aromatic"] = 4.1 / 2.0 #4.3 / 2.0    #sigma for ring
    bead_params["rvdw_cross"] = 0.5 * ((4.7 / 2.0) + (4.3 / 2.0))
    bead_params["offset_bd_weight"] = 30.0 #50.0    #penalty weight for nonring beads
    bead_params["offset_bd_aromatic_weight"] = 10.0 #20.0    #penalty weight for ring beads
    ...
    for num_beads in range(1, max_beads + 1):
        ...
        for seq in seq_one_beads:
            ...
            if acceptable_trial:
                ...
                if all_atoms_in_beads_connected(trial_comb, heavyatom_coords, list_heavy_atoms, list_bonds, ring_atoms):
                    # Accept the move
...
def all_atoms_in_beads_connected(trial_comb, heavyatom_coords, list_heavyatoms, bondlist, ring_atoms):
    ...
    voronoi = voronoi_atoms(cgbead_coords, heavyatom_coords, ring_atoms)
                ...
                for j in range(len(heavyatom_coords)):
                    if partitioning[j] != lonely_bead:
                        ...
                        if dist_bead_at < closest_bead_dist and \
                           ...
                           sum(x == partitioning[j] for x in partitioning.values()) > 1: # was "> 2"
                            closest_bead = j
